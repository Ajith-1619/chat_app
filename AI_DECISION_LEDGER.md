...
## 2026-08-05 - Horizon latency decision
- Decision: fix the dependency order and database connection behavior instead of increasing the client timeout.
- Reason: a longer timeout would hide the remote DB bottleneck and make the UI feel slower; optional task/location enrichment must not block the attendance summary.
...
# AI-DECISION-2026-08-06-HORIZON
- Kept the separate all-employees map page and used a bounded modal for employee routes. Reused existing OSM/ArcGIS providers because no Google Maps key was provided.


# AI-DECISION-2026-08-06-HORIZON-MODAL
- Kept map point time labels inside the map and removed only the modal's redundant timeline, preserving the standalone route page.


# AI-DECISION-2026-08-06-MYHUB-WORKSPACE-UI
- Chose a responsive Flutter grid rather than separate mobile/desktop screens, preserving current navigation while matching the supplied mobile card layout.
- Kept the existing Workspace filter set and added scrolling affordance instead of changing filter semantics.
