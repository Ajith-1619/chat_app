...
function flow_api_ext_chat(array $auth, array $segments): never
{
    ...
    if (($segments[0] ?? '') === 'direct' && ($segments[1] ?? '') === 'messages') {
        if ($method === 'GET') {
            $peerEmpId = (int)($_GET['recipient_emp_id'] ?? $_GET['to_emp_id'] ?? $_GET['peer_emp_id'] ?? 0);
            $senderEmpId = (int)($_GET['sender_emp_id'] ?? $_GET['from_emp_id'] ?? $auth['actor_emp_id']);
        ...
        if ($method === 'POST') {
            ...
            if ($recipientEmpId <= 0) flow_api_error('recipient_emp_id is required.', 422, 'VALIDATION_ERROR', ['debug' => $inputDebug]);
            if ($senderEmpId <= 0) flow_api_error('sender_emp_id is invalid.', 422, 'VALIDATION_ERROR', ['debug' => $inputDebug]);
            if ($body === '') flow_api_error('body is required.', 422, 'VALIDATION_ERROR', ['debug' => $inputDebug]);
