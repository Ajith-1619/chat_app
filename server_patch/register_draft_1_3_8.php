...
$builds = [
    ['android', '1.3.8', 17, 'https://dns.watchtower247.in/router_login/downloads/Skylink-Chat-v1.3.8.apk'],
    ['windows', '1.3.8', 17, 'https://dns.watchtower247.in/router_login/downloads/Skylink-Chat-Setup-v1.3.8.exe'],
    ['linux', '1.3.8', 17, 'https://dns.watchtower247.in/router_login/downloads/skylink-chat_1.3.8_amd64.deb'],
];
...
foreach ($builds as $build) {
    ...
    $stmt->execute([
        ...
        ':url' => $url,
        ':notes' => 'Latency and attendance hotfix uploaded as Draft. Requires Ajith (302) approval before production.',
    ]);
    ...
    $hist = $pdo->prepare(
        "INSERT INTO xmpp_release_history
         ...
         'Auto-registered draft after latency hotfix artifact upload')"
    );
    $hist->execute([':id' => $id]);
