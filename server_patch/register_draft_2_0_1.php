...
$notes = implode("\n", [
    'v2.0.1 Android draft build.',
    'Includes current/live location map-card rendering, Android text-send correction, task creation persistence, and group/channel member mention fixes.',
    'This build is Draft only. Force update is disabled. Production deployment requires Ajith (Employee ID 302) approval.',
]);

$builds = [
    ['android', '2.0.1', 24, 'https://dns.watchtower247.in/router_login/downloads/Skylink-Chat-v2.0.1.apk'],
];
...
foreach ($builds as $build) {
    ...
    $hist = $pdo->prepare(
        "INSERT INTO xmpp_release_history
         ...
         'Auto-registered v2.0.1 Android draft after APK upload')"
    );
    $hist->execute([':id' => $id]);
