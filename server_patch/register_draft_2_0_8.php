...
$notes = implode("\n", [
    'v2.0.8 Android/Web draft build generated on 2026-07-30.',
    'Includes next-action owner-response prompt workflow: Complete requires update notes, Not complete requires next action date.',
    'Includes repeated next-action due prompt reminders until the assigned owner responds.',
    'Includes recent Flow chat, API, attachment, Horizon, slash command, channel metadata, and operational updates present in the workspace build.',
    'Artifacts include Web ZIP and Android release APK.',
...
$builds = [
    ['android', '2.0.8', 31, 'https://dns.watchtower247.in/router_login/downloads/Skylink-Chat-v2.0.8.apk'],
];
...
foreach ($builds as $build) {
    ...
    if ($id > 0) {
        ...
        $hist->execute([
            ':id' => $id,
            ':notes' => 'Auto-registered v2.0.8 ' . $platform . ' draft after artifact upload',
        ]);
    }
